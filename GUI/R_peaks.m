function [qrs_i_raw,qrs_amp_raw,locs,NOISL_buf1,SIGL_buf1,THRS_buf1] = R_peaks(signal,signal_1,fs)
...
if fs ~= 360
 ...
 signal_1 = filtfilt(b,1,signal_1);
 signal_1 = signal_1/max(signal_1);

 
 
 %% ==========  Cuadratura de forma no lineal mejora los picos dominantes ========== %%
 signal_1 =  signal_1.^2;

 
 %% ============  Integracion Ventana Movil ================== %%
%-------Y(nt) = (1/N)[x(nT-(N - 1)T)+ x(nT - (N - 2)T)+...+x(nT)]---------%
signal_1 = conv(signal_1 ,ones(1 ,round(0.150*fs))/round(0.150*fs));
